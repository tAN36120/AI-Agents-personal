# Referenced from: https://github.com/gortizji/tangent_task_arithmetic/blob/main/src/args.py
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Specify the auto augmentation strategy. The format is 'rand-m${m}-n${n}-mstd0.5'",
    )
    ...
    parser.add_argument(
        ...
        type=lambda x: x.split(","),
        default=["Cars", "SVHN"],
        help="Datasets to unlearn/merge (comma-separated). Default: Cars,SVHN",
    )

    parser.add_argument(
        "--layer-wise-zeroing",
        action="store_true",
        help="Enable Experiment 4: Force task vectors of shallow layers to zero.",
    )
    parser.add_argument(
        "--shallow-threshold",
        type=int,
        default=3,
        help="Transformer blocks <= this index will be zeroed out (default 3, matches 'Shallow' in Table 6).",
    )
